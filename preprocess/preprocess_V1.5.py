...
class SemanticAnalyzer:
    ...
    def calculate_coherence(self, conversation):
        ...
        # 提取所有非系统消息
        messages = [msg["content"] for msg in conversation["messages"] if msg["role"] != "system"]
        if len(messages) < 2:
            return 0.0
...
class EnhancedConversationBuilder:
    ...
    def build(self):
        ...
        conversation = {"messages": formatted, "metadata": metadata}

        # 检查整体连贯性
        if not self.semantic_analyzer.is_coherent(conversation):
...
def process_csv_to_jsonl(input_csv, output_jsonl):
    ...
    try:
        ...
        # 4. 生成质量报告
        generate_quality_report(valid_conversations)
...
def generate_quality_report(conversations):
    ...
    # 分析对话长度
    turn_counts = [conv["metadata"]["turn_count"] for conv in conversations]
    avg_turns = np.mean(turn_counts)
    max_turns = max(turn_counts)
    min_turns = min(turn_counts)

    # 分析参与者数量
    participant_counts = [len(conv["metadata"]["participants"]) for conv in conversations]
    avg_participants = np.mean(participant_counts)
    ...
    for idx in sample_indices:
        score = semantic_analyzer.calculate_coherence(conversations[idx])
...
对话轮次统计: 平均={avg_turns:.1f}, 最小={min_turns}, 最大={max_turns}
参与者统计: 平均={avg_participants:.1f} 人
连贯性得分: {avg_coherence:.2f} (基于 {sample_size} 个样本)
"""
    logger.info(report)

    # 保存报告
    with open("conversation_quality_report.txt", "w", encoding="utf-8") as f:
        f.write(report)
...
if __name__ == "__main__":
    ...
    if process_csv_to_jsonl(input_file, output_file):
        ...
        print(f"质量报告已保存到: conversation_quality_report.txt")
    else:
