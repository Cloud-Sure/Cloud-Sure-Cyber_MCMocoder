...
class EnhancedConversationBuilder:
    ...
    def add_message(self, role, content, timestamp, sender):
        ...
        # 计算筛选后用户消息与助理回复的相似度
        if self.semantic_analyzer.initialized:
            pair_score = self.semantic_analyzer.calculate_similarity(user_content, content)
            logger.debug(f"对话对相似度: {pair_score:.2f}")

            # 如果相似度低于阈值，跳过这个对话对
            if pair_score < Config.MIN_PAIR_SCORE:
                logger.debug(f"对话对相关性不足 ({pair_score:.2f} < {Config.MIN_PAIR_SCORE}), 跳过")
                # 重置用户消息缓存，但保留之前的消息
...
def process_csv_to_jsonl(input_csv, output_jsonl):
    ...
    try:
        ...
        valid_conversations = []
        message_count = 0
        skipped_pairs = 0  # 记录跳过的对话对数量

        for _, row in df.iterrows():
            message_count += 1
            if message_count % 1000 == 0:
                logger.info(
                    f"已处理 {message_count} 条消息，找到 {len(valid_conversations)} 个有效对话, 跳过 {skipped_pairs} 个低质量对话")
            ...
            # 尝试添加消息
            result = builder.add_message(role, content, current_time, sender)
            if not result:
                # 如果是低质量对话对，增加计数
                if role == 'assistant':
                    skipped_pairs += 1
        ...
        logger.info(f"处理完成! 生成 {len(valid_conversations)} 个高质量对话, 跳过 {skipped_pairs} 个低质量对话对")
...
if __name__ == "__main__":
    ...
    output_file = r"D:\GitHub\Cloud-Sure-Cyber_MCMocoder\jsonl\high_quality_output_1.7.jsonl"
